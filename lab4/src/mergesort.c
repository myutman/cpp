...
int mergesort (void* base, size_t num, size_t size, int(*cmp)(const void*, const void*)){
	...
	while (i < n1 || j < num){
		...
		if (j == num || ((i < n1) && ((int)cmp((const void*)(base + i * size), (const void*)(base + j * size)) < 0))){
			//printf("hello\n");
			copy((char*)(an + k), (char*)(base + ((i++) * size)), size);
			k += size;
		}
		else{
			//printf("hello\n");
			copy((char*)(an + k), (char*)(base + ((j++) * size)), size);
			k += size;
		}
		//printf("hello\n");
		//printf(" %d\n", *((int*) (an + k - size)));
	...
	for (i = 0; i < num * size; i += size)
		copy((char*)(base + i), (char*)(an + i), size);
	free(an);
}
